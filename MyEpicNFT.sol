...
contract MyEpicNFT is ERC721URIStorage {
  ...
  string[] firstWords = ["terra", "sol", "lua", "urano", "saturno", "marte"];
  string[] secondWords = ["estrela", "galaxia", "planeta", "buraconegro", "astronauta"];
  string[] thirdWords = ["aries", "sagitario", "capricornio", "gemos", "touro", "leao"];

  event NewEpicNFTMinted(address sender, uint256 tokenId);

  constructor() ERC721 ("UniversoNFT", "UNIVERSO") {
  ...
  function pickRandomFirstWord(uint256 tokenId) public view returns (string memory) {
    ...
    uint256 rand = random(string(abi.encodePacked("terra", Strings.toString(tokenId))));
  ...
  function pickRandomSecondWord(uint256 tokenId) public view returns (string memory) {
    uint256 rand = random(string(abi.encodePacked("estrela", Strings.toString(tokenId))));
    rand = rand % secondWords.length;
    return secondWords[rand];
  }

  function pickRandomThirdWord(uint256 tokenId) public view returns (string memory) {
    uint256 rand = random(string(abi.encodePacked("aries", Strings.toString(tokenId))));
